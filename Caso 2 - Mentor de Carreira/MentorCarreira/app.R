...
ui <- fluidPage( theme = shinytheme("superhero"),
    ...
    fluidRow( 
        ...
        column(4, sliderInput("scomunicar", "Habilidades de Comunição", min = 40, max = 75, step = 5, value = 50) )
    ),
    fluidRow(
        column(6, h1("Sistema"), plotOutput("GrafSistema")),
        column(6, actionButton("Processar", "Processar"), 
               helpText("A linha Vermelha mostra a sua aderêcia a profissão de Cientista de Dados"),
...
server <- function(input, output) {
    ...
    observeEvent(input$Processar, {
        inferencia <- fuzzy_inference(sistema, list(Gosta_exatas = input$sexatas, Rel_interpess = input$sinter, 
                                                    Escre_codigo = input$scodigo, Pefil_lider = input$slider, 
                                                    Gosta_Estudar = input$sestudar, Habilidade_comunicao = input$scomunicar))
