...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Otimização de Transporte de Carga"),
...
server <- function(input, output) {
    ...
    observeEvent(input$Totais, {
        ...
        # conta o nº de linhas do df
        z <<- nrow(itens)
        
        output$Dados = renderTable( { itens} )
        # paste0() -> serve para concatenar texto
        output$TQuantidade = renderText( {paste0("Quantidade de Itens: ", z)} )
        output$TPesototal = renderText( {paste0("Peso Total da Carga: ", sum(itens$PESO))} )
    ...
    observeEvent(input$Processar, {
        ...
        # ga("binary", -> do tipo binário
        # ftness ->  função de avaliação
        # popSize -> população
